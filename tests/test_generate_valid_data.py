...
def test_generate_valid_data(file_number: int, sieve, cleanup_files_session):
    ...
    results = [sieve.calculator.is_prime_optimized(data[random.randint(1, size)]) for _ in range(division_factor)]
    results.extend([sieve.calculator.is_prime_optimized(data[x]) for x in range(5)])
    results.extend([sieve.calculator.is_prime_optimized(data[-x]) for x in range(1, 5)])
...
def test_optimized_method_faster_than_basic_method(file_number: int, cleanup_files_session, sieve):
    ...
    results_basic = [sieve.calculator.is_prime_optimized(data[x]) for x in range(5)]
